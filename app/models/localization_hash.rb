class LocalizationHash < Hash
  ...
  def self.init_from_yaml(yaml_dir, hash_dir, langs)
    # Initialize the hash from all the yaml localization files in dir that have data for the languages given by langs
    ...
    data = {}
    langs.each {|lang|
      data[lang] = YAML::load( File.open(yaml_dir+"/"+lang+'.yml' ))[lang].sort_by_key(true)
    ...
    File.open(hash_dir+'/interpretatio_hash.rb', "w") {|file| file.puts @mega.inspect }
  end
